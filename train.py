...
writer = SummaryWriter(log_dir="Scut_tboard",comment='Linear')
...
if args.cuda:
    if args.multigpu:
        net = torch.nn.DataParallel(s3fd_net,device_ids=device_ids)
    net = net.cuda()
...
if not args.resume:
    ...
    s3fd_net.extra_pre.apply(s3fd_net.weights_init)
    s3fd_net.combine_mod.apply(s3fd_net.weights_init)
...
def train():
    ...
    for epoch in range(start_epoch, cfg.EPOCHES):
        ...
        for batch_idx, (images, targets) in enumerate(train_loader):
            if args.cuda:
                images = Variable(images.cuda())                    #batch_size,3,640,640
                targets = [Variable(ann.cuda(), volatile=True)      #batch_size,head_num,5(loc,conf=1)
                           for ann in targets]
            else:
            ...
            # backprop
            optimizer.zero_grad()
            loss_l, loss_c = criterion(out, targets)                #out:[loc,conf,prior]
            loss = loss_l + loss_c
            loss.backward()
            optimizer.step()
            t1 = time.time()
            ...
            if iteration % 10 == 0:
                ...
                writer.add_scalar('Train_loss', tloss, iteration)
                writer.add_scalar('conf_loss', loss_c.data[0], iteration)
                writer.add_scalar('loc_loss', loss_l.data[0], iteration)
...
def val(epoch,net):
    ...
    writer.add_scalar('val_loss', tloss, epoch)
...
def adjust_learning_rate(optimizer, gamma, step):
    ...
    orilr=[0.001,0.0005,0.00025,0.0001]
    lr=orilr[step]
