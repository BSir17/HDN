...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        priors = priors[:loc_data.size(1), :]               #shape:[feature_maps的特征点的总数量,4]
        num_priors = (priors.size(0))                       #feature_maps的特征点的总数量,也是先验框的数量
        ...
        for idx in range(num):
            truths = targets[idx][:, :-1].data              #shape:[face_num in a picture,4]
            labels = targets[idx][:, -1].data               #shape:[face_num in a picture]
