...
parser = argparse.ArgumentParser(description='s3fdx evaluatuon wider')
parser.add_argument('--model', type=str,
                    default='../weights/wider_train2/sfd_face.pth', help='trained model')
...
sys.stdout = Logger('wider_val_record.txt')
...
cpunet = build_s3fd('test', cfg.NUM_CLASSES)
cpunet.load_state_dict(torch.load(args.model))
cpunet.eval()
...
if __name__ == '__main__':
    ...
    netgpu = build_s3fd('test', cfg.NUM_CLASSES)
    netgpu.load_state_dict(torch.load(args.model))
    ...
    for index, event in enumerate(event_list):
        ...
        for num, file in enumerate(filelist):
            ...
            im_name = str(file[0][0])
            in_file = os.path.join(imgs_path, str(event[0][0]), im_name[:] + '.jpg')
            ...
            t1 = time.time()
            print(img.shape)
            if img.shape[0]*img.shape[1]<600*1024 or img.shape[0]*img.shape[1]>1578*1024:
                net=cpunet
                use_cuda = False
                torch.set_default_tensor_type('torch.FloatTensor')
            ...
            det = np.row_stack((det0, det2, det3))
            dets = bbox_vote(det)
